...
Swing JLabel을 이용한 계산기

```java
public class Calc extends JFrame {

    static JLabel label; // 화면 중앙에 띄어지는 숫자
    static JLabel info; // 화면 상단에 현재 상황 출력
    static int flag = 0; // 계산 결과가 양수인지 체크
    static int check = 0; // 1000만 미만의 수인지 체크

    public Calc() {
        setTitle("계산기");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 이 프로그램의 제목, 높이와 너비 등을 레이아웃 기본 설정
    ...
    class NorthPanel extends JPanel {
        public NorthPanel() {
            setLayout(new GridLayout(3,1)); // 이 프로그램의 위쪽 부분 숫자와 상황을 출력하는 곳 기본 설정
    ...
    예시![ㅇㅇ](https://ifh.cc/g/IcfYuT.png)
    ...
    class CenterPanel extends JPanel {
        public CenterPanel() {
            ...
            bt[4] = new JButton("4");
            bt[5] = new JButton("5");
            bt[6] = new JButton("6"); // 배열을 이용해 숫자보드 설정
            bt[7] = new JButton("×");
